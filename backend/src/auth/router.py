...
from auth.schemas import SelfUserUpdate, UserRead, UserUpdate, SoloUserRegister, TeamUserRegister, UserCreate, TeamUserCreate
from auth.users import conf, get_user_or_admingeneral, current_active_user, get_user_manager, UserManager
from olymp.models import TeamMembers, RegisteredOlymp, ExpertTaskAssignment
from olymp.schemas import TeamMembersRead
from olymp.methods import delete_answers_by_user
...
async def get_user(user: User = Depends(current_active_user), session: AsyncSession = Depends(get_async_session)):
    ...
    result = await session.execute(select(TeamMembers).filter(TeamMembers.team_id == user.id))
    ...
    if user_to_update.role_id == 6:
        if update_dict.members:
            for member_data in update_dict.members:
                ...
                if not member:
                    raise HTTPException(status_code=404, detail=f"Участник команды не найден")
...
async def custom_register(
    ...
    user_data = user_create.dict(exclude={"olymp_id"})
    user_data["role_id"] = 5
